...
module "ec2-key-pair" {
  ...
  path = "${var.envs_path}/${var.env_folder_name}/vpn-ssh-${var.default_tags["_Environment"]}.pub"
}
...
resource "aws_instance" "web" {
  ...
  key_name      = module.ec2-key-pair.key_name
...
##Extenrnal CNAME
module "az_cname_entry"{
  source          = "../../modules-azure/m-dns-cname"
  az_dns_name     = var.az_dns_name
  az_rg_name      = var.az_rg_name
  dns_name        = "${var.az_dns_vpn}.${var.default_tags["Infra-Unit"]}"
  dns_targed_name = aws_eip.one.public_dns
  extra           = aws_instance.web.id
}
