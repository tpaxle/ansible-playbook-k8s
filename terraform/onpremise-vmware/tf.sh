...
if [[ -z "${INFRA_UNIT_DIR}" ]]; then
  INFRA_UNIT_DIR="$(pwd)/src/projects/platform-public/"

fi


CONFIG_DIR="$(pwd)/src/envs"
...
load_backend_config(){
  AWS_ROLE=$(get_hcl_value role_arn)
...
init(){
  ...
  cd $INFRA_UNIT_DIR/$INFRA_UNIT && TF_PLUGIN_CACHE_DIR=$TF_PLUGIN_CACHE_DIR TF_DATA_DIR=$TF_DATA_DIR terraform init -backend-config=$CONFIG_DIR/$ENV/backend-$ENV.tfvars -var-file=$CONFIG_DIR/tosecure/azure.tfvars >> ${INFRA_UNIT_DIR}/${INFRA_UNIT}/terraform_init.log
}
...
run_tf(){
  [[ -z $FAST ]] && init
  command="cd $INFRA_UNIT_DIR/$INFRA_UNIT && TF_PLUGIN_CACHE_DIR=$TF_PLUGIN_CACHE_DIR TF_DATA_DIR=$TF_DATA_DIR terraform $ACTION $AUTOAPPROVE $AUTORUN -var-file=$CONFIG_DIR/$ENV/$ENV.tfvars -var-file=$CONFIG_DIR/tosecure/azure.tfvars -var-file=$CONFIG_DIR/$ENV/backend-$ENV.tfvars ${EXTRA_ARGS[@]}"
  echo "Running the following command next:"
  echo $command
  eval $command
}
...
run_graph(){
  init
  command="cd $INFRA_UNIT_DIR/$INFRA_UNIT && TF_PLUGIN_CACHE_DIR=$TF_PLUGIN_CACHE_DIR TF_DATA_DIR=$TF_DATA_DIR terraform $ACTION -draw-cycles | dot -Tsvg > plan_graph.svg"
  #echo "Running the following command next:"
  #echo $command
  eval $command
}
